...
import city.roles.RestaurantZhangWaiterBase;
import city.roles.RestaurantZhangWaiterBase.MyCustomer;
...
public class RestaurantZhangWaiterSharedDataTest extends TestCase {
	...
	public void testOneNormalWaiterScenario() { // Tests when customer has more than enough money to pay
		...
		assertEquals("Waiter's customer's state should be waiting. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.waiting, waiter.myCustomerList.get(0).state);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be null. It isn't", null, waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be deciding. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.deciding, waiter.myCustomerList.get(0).state);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be null. It isn't", null, waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be readyToOrder. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.readyToOrder, waiter.myCustomerList.get(0).state);
		assertTrue("Waiter's customer's log is incorrect. It reads: " + customer.log.getLastLoggedEvent().toString(), customer.log.getLastLoggedEvent().toString().contains("Waiter Waiter seated me at table 0"));
		assertEquals("Waiter should have one for number of customers served. It's instead " + waiter.numberCustomersServed, 1, waiter.numberCustomersServed);
		assertTrue("Waiter's checkList should be empty. It isn't", waiter.checkList.isEmpty());
		assertEquals("Waiter's break status should be notOnBreak. It's instead " + waiter.wBreakStatus, RestaurantZhangWaiterBase.breakStatus.notOnBreak, waiter.wBreakStatus);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be null. It isn't", null, waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be ordering. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.ordering, waiter.myCustomerList.get(0).state);
		...
		assertEquals("Waiter's customer's state should be ordered. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.ordered, waiter.myCustomerList.get(0).state);
		assertTrue("Waiter's customer's log is incorrect. It reads: " + customer.log.getLastLoggedEvent().toString(), customer.log.getLastLoggedEvent().toString().contains("Waiter asked what I would like"));
		assertEquals("Waiter should have one for number of customers served. It's instead " + waiter.numberCustomersServed, 1, waiter.numberCustomersServed);
		assertTrue("Waiter's checkList should be empty. It isn't", waiter.checkList.isEmpty());
		assertEquals("Waiter's break status should be notOnBreak. It's instead " + waiter.wBreakStatus, RestaurantZhangWaiterBase.breakStatus.notOnBreak, waiter.wBreakStatus);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be Steak. It isn't", "Steak", waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be orderCooking. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.orderCooking, waiter.myCustomerList.get(0).state);
		assertTrue("Waiter's customer's log is incorrect. It reads: " + customer.log.getLastLoggedEvent().toString(), customer.log.getLastLoggedEvent().toString().contains("Waiter asked what I would like"));
		assertEquals("Waiter should have one for number of customers served. It's instead " + waiter.numberCustomersServed, 1, waiter.numberCustomersServed);
		assertTrue("Waiter's checkList should be empty. It isn't", waiter.checkList.isEmpty());
		assertEquals("Waiter's break status should be notOnBreak. It's instead " + waiter.wBreakStatus, RestaurantZhangWaiterBase.breakStatus.notOnBreak, waiter.wBreakStatus);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be Steak. It isn't", "Steak", waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be orderReady. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.orderReady, waiter.myCustomerList.get(0).state);
		assertTrue("Waiter's customer's log is incorrect. It reads: " + customer.log.getLastLoggedEvent().toString(), customer.log.getLastLoggedEvent().toString().contains("Waiter asked what I would like"));
		assertEquals("Waiter should have one for number of customers served. It's instead " + waiter.numberCustomersServed, 1, waiter.numberCustomersServed);
		assertTrue("Waiter's checkList should be empty. It isn't", waiter.checkList.isEmpty());
		assertEquals("Waiter's break status should be notOnBreak. It's instead " + waiter.wBreakStatus, RestaurantZhangWaiterBase.breakStatus.notOnBreak, waiter.wBreakStatus);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be Steak. It isn't", "Steak", waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be orderReady. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.eating, waiter.myCustomerList.get(0).state);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be Steak. It isn't", "Steak", waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be doneEating. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.doneEating, waiter.myCustomerList.get(0).state);
		assertEquals("Waiter should have one for number of customers served. It's instead " + waiter.numberCustomersServed, 1, waiter.numberCustomersServed);
		assertTrue("Waiter's checkList should be empty. It isn't", waiter.checkList.isEmpty());
		assertEquals("Waiter's break status should be notOnBreak. It's instead " + waiter.wBreakStatus, RestaurantZhangWaiterBase.breakStatus.notOnBreak, waiter.wBreakStatus);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be Steak. It isn't", "Steak", waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be waitingForCheck. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.waitingForCheck, waiter.myCustomerList.get(0).state);
		assertTrue("Waiter's customer's log is incorrect. It reads: " + customer.log.getLastLoggedEvent().toString(), customer.log.getLastLoggedEvent().toString().contains("Waiter gave me my food Steak"));
		assertEquals("Waiter should have one for number of customers served. It's instead " + waiter.numberCustomersServed, 1, waiter.numberCustomersServed);
		assertTrue("Waiter's checkList should be empty. It isn't", waiter.checkList.isEmpty());
		assertEquals("Waiter's break status should be notOnBreak. It's instead " + waiter.wBreakStatus, RestaurantZhangWaiterBase.breakStatus.notOnBreak, waiter.wBreakStatus);
		...
		assertEquals("Waiter's customer's table should be set. It isn't", table, waiter.myCustomerList.get(0).table);
		assertEquals("Waiter's customer's choice should be Steak. It isn't", "Steak", waiter.myCustomerList.get(0).choice);
		assertEquals("Waiter's customer's state should be leaving. Instead it's " + waiter.myCustomerList.get(0).state, MyCustomer.STATE.leaving, waiter.myCustomerList.get(0).state);
		assertTrue("Waiter's customer's log is incorrect. It reads: " + customer.log.getLastLoggedEvent().toString(), customer.log.getLastLoggedEvent().toString().contains("Received check from waiter"));
		assertEquals("Waiter should have one for number of customers served. It's instead " + waiter.numberCustomersServed, 1, waiter.numberCustomersServed);
		assertTrue("Waiter's checkList should be empty. It isn't", waiter.checkList.isEmpty());
		assertEquals("Waiter's break status should be notOnBreak. It's instead " + waiter.wBreakStatus, RestaurantZhangWaiterBase.breakStatus.notOnBreak, waiter.wBreakStatus);
